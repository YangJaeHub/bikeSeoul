...
def get_distance_from_han_river(tuple):
    ...
    if lat < calculate_coor_list[0][0]:    # 김포대교 ~ 여의도쪽 두껍게 시작
        ...
        point = get_coefficient_two_dimension(calculate_coor_list[3], calculate_coor_list[4])
        nearest_point = get_nearest_point_from_graph_two_dimension(point[0], point[1], lat, lon)
        ...
        point = get_coefficient_two_dimension(calculate_coor_list[8], calculate_coor_list[9])
        nearest_point = get_nearest_point_from_graph_two_dimension(point[0], point[1], lat, lon)
    elif lat < calculate_coor_list[9][0]:  # 천호대교 ~ 구리암사대교 (near -1 / 5 x^)get_coefficient_two_dimension
        point = get_coefficient_two_dimension(calculate_coor_list[9], calculate_coor_list[10])
        nearest_point = get_nearest_point_from_graph_two_dimension(point[0], point[1], lat, lon)
    else:   # 구리암사대교 ~ 끝
